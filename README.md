# 💫 Hi. I'm Akshy M Pillai.
 I'm a Microsoft Certified Data Analyst **[PL-300]** from India. I'm an MBA Graduate. I have hands on experience building multiple End-to-End reports and dashboard in PowerBi. My Skills include but not limited to: 
 - Getting raw data from various sources ranging from a simple .csv file or an SQL server to a Data Lakehouse in **Microsoft Fabric**.
 - Securing a report with **RLS**. Creating and managing a workspace by assigning user roles and security groups.
 - Extracting useful insights for Leadership and highlighting them without clutter. Creating a report with best design practices endorsed by microsoft.
  

🎯 **Data Analyst | SQL • Power BI • Python | DAX Enthusiast**  
📍 India  

I turn raw data into clear insights and practical dashboards. Currently building advanced analytics projects focused on e-commerce, customer behavior, and automation.

## 🔧 Tech Stack
**Languages:** SQL, Python, DAX  
**Databases:** SQL Server, PostgreSQL  
**Analytics & BI:** Power BI, SSMS, Excel  
**Tools:** Git, Jupyter, VS Code  

## 📊 Featured Projects
- 📦 **E-commerce Analytics Dashboard** – Customer cohorting, RFM, funnel drop-off analysis  
- 🧠 **Sentiment Analysis with Transformers** – NLP on customer reviews with Roberta  
- 📈 **Advanced Power BI Portfolio** – DAX modeling, time intelligence, event tracking  
- 🔄 **Direct Query SQL Pipelines** – Converting raw event logs into analytics-ready tables  

## 📚 Currently Learning
- Data Modeling (Star Schema, Fact/Dim)
- Power BI performance optimization  
- Python for ML workflows 



### 🌐 Socials:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/akshympillai) [![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:akshym.official@gmail.com) 

# 💻 Tech Stack:
![Python](https://img.shields.io/badge/python-3670A0?style=plastic&logo=python&logoColor=ffdd54) ![MicrosoftSQLServer](https://img.shields.io/badge/Microsoft%20SQL%20Server-CC2927?style=plastic&logo=microsoft%20sql%20server&logoColor=white) ![Figma](https://img.shields.io/badge/figma-%23F24E1E.svg?style=plastic&logo=figma&logoColor=white) ![Canva](https://img.shields.io/badge/Canva-%2300C4CC.svg?style=plastic&logo=Canva&logoColor=white) ![Pandas](https://img.shields.io/badge/pandas-%23150458.svg?style=plastic&logo=pandas&logoColor=white) ![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=plastic&logo=github&logoColor=white) ![Power Bi](https://img.shields.io/badge/power_bi-F2C811?style=plastic&logo=powerbi&logoColor=black)![ChatGPT](https://img.shields.io/badge/chatGPT-74aa9c?style=plastic&logo=openai&logoColor=white)![Visual Studio Code](https://img.shields.io/badge/Visual%20Studio%20Code-0078d7.svg?style=plastic&logo=visual-studio-code&logoColor=white)

### ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->


